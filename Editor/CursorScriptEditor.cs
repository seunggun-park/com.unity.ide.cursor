...
{
    [InitializeOnLoad]
    public class CursorScriptEditor : IExternalCodeEditor
    {
        const string cursor_argument = "cursor_arguments";
        const string cursor_extension = "cursor_userExtensions";
        ...
        string Arguments
        {
            get => m_Arguments ?? (m_Arguments = EditorPrefs.GetString(cursor_argument, DefaultArgument));
            set
            {
                m_Arguments = value;
                EditorPrefs.SetString(cursor_argument, value);
            }
        }
        ...
        {
            get => EditorPrefs.GetString(cursor_extension, string.Join(";", defaultExtensions));
            set => EditorPrefs.SetString(cursor_extension, value);
        }
        ...
        public CursorScriptEditor(IDiscovery discovery, IGenerator projectGeneration)
        {
            m_Discoverability = discovery;
            m_ProjectGeneration = projectGeneration;
        }

        static CursorScriptEditor()
        {
            var editor = new CursorScriptEditor(new CursorDiscovery(), new ProjectGeneration(Directory.GetParent(Application.dataPath).FullName));
            CodeEditor.Register(editor);

            if (IsCursorInstallation(CodeEditor.CurrentEditorInstallation))
            {
                editor.CreateIfDoesntExist();
            }
        }

        static bool IsCursorInstallation(string path)
        {
